...
(function () {
  ...
  var TRANSLATIONS = {
    en: {
      ...
      "trust.lede": "sha256 alone proves integrity, not authenticity — the same document that points to a skill also asserts its hash. Signing closes that gap. And neither says whether the content is still true — signed freshness covers that.",
      ...
      "trust.card3H3": "Freshness — signed “still true”",
      "trust.card3P__html": "Hashes freeze bytes, not truth. A registered reviewer signs a freshness attestation bound to the exact content sha — voided by any edit, expiring on a date — and <code>llms-skills freshness</code> turns stale knowledge into a failing CI build.",
    ...
    es: {
      ...
      "trust.lede": "El sha256 solo prueba integridad, no autenticidad — lo asevera el mismo documento que apunta a la skill. Firmar cierra esa brecha. Y ninguno dice si el contenido sigue siendo verdad — eso lo cubre la vigencia firmada.",
      ...
      "trust.card3H3": "Vigencia — “sigue siendo verdad”, firmado",
      "trust.card3P__html": "Los hashes congelan bytes, no verdad. Un revisor registrado firma una atestación de vigencia ligada al sha exacto del contenido — se anula con cualquier edición y caduca en una fecha — y <code>llms-skills freshness</code> convierte conocimiento vencido en un build rojo de CI.",
    ...
    pt: {
      ...
      "trust.lede": "O sha256 sozinho prova integridade, não autenticidade — quem afirma isso é o mesmo documento que aponta para a skill. Assinar fecha essa brecha. E nenhum diz se o conteúdo continua verdadeiro — isso é coberto pela vigência assinada.",
      ...
      "trust.card3H3": "Vigência — “continua verdadeiro”, assinado",
      "trust.card3P__html": "Hashes congelam bytes, não verdade. Um revisor registrado assina uma atestação de vigência ligada ao sha exato do conteúdo — anulada por qualquer edição e expirando numa data — e <code>llms-skills freshness</code> transforma conhecimento vencido em um build vermelho no CI.",
